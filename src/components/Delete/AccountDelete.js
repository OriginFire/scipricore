...
export default function AccountDelete(props) {
    ...
    const accountSignIn = async () => {
        try {
            const user = await deleteUser(email, secretPassword);
            let axiosConfig = {
                headers: {
            ...
            axios.delete(`https://scipricore-backend.herokuapp.com/user/${user.user.uid}`, axiosConfig)
            ...
            >
                <div className="selection">
                    <p>
                        ...
                        <code className={menu === 0 ? "blue" : ""}>Delete Account</code>
                    </p>
                </div>
